...
from PyQt5.QtGui import QColor, QFont, QImage, QKeySequence, QPainter, QPainterPath, QPen, QPixmap, QPolygonF
...
class ShortcutDialog(QDialog):
    ...
    def accept(self):
        ...
        for key, value in values.items():
            normalized = QKeySequence(value).toString(QKeySequence.NativeText)
            if not normalized:
                continue
...
class Canvas(QWidget):
    ...
    def resize_annotation(self, annotation, handle_name, point_x, point_y):
        box = annotation.normalized()
        if "l" in handle_name:
            box.x1 = point_x
        if "r" in handle_name:
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.mode == "resize" and self.selected_annotation():
            annotation = self.selected_annotation()
            self.resize_annotation(annotation, self.active_handle, image_x, image_y)
    ...
    def copy_selected(self):
        annotation = self.selected_annotation()
        if annotation:
            self.clipboard_annotation = Annotation(annotation.x1, annotation.y1, annotation.x2, annotation.y2, annotation.class_id, annotation.score)

    def paste_annotation(self):
        if not self.clipboard_annotation or not self.has_image():
            return
        self.push_history()
        clone = Annotation(
            self.clipboard_annotation.x1 + 12,
            self.clipboard_annotation.y1 + 12,
            self.clipboard_annotation.x2 + 12,
            self.clipboard_annotation.y2 + 12,
            self.clipboard_annotation.class_id,
            self.clipboard_annotation.score,
        )
    ...
    def nudge_selected_group(self, dx, dy):
        ...
        for index in indices:
            if 0 <= index < len(self.annotations):
                ...
                self.annotations[index].clamp(self.image.shape[1], self.image.shape[0])
        self.annotations_changed.emit()
        self.update()
    ...
    def copy_previous_annotations(self, previous_annotations):
        if not previous_annotations:
            return
        self.push_history()
        self.annotations = [Annotation(item.x1, item.y1, item.x2, item.y2, item.class_id, item.score) for item in previous_annotations]
        self.set_selected_index(0 if self.annotations else -1)
        self.annotations_changed.emit()
        self.update()
...
class Annotator(QMainWindow):
    def __init__(self):
        ...
        self.few_shot_status = QLabel("Few-shot assist: ready")
        self.few_shot_status.setWordWrap(True)
        self.assist_notes = QTextEdit()
        self.assist_notes.setReadOnly(True)
        self.assist_notes.setPlainText(
            "\n".join(
                [
                    "Assist modes:",
                    "- Detector Assist: uses your loaded .pt detection model to create box suggestions.",
                    "- Few-Shot Assist: uses labeled images in this project as a support pool and transfers boxes from the best visual matches.",
                    "",
                    "Recommended workflow:",
                    "1. Start with manual labels on a few images.",
                    "2. Use Detector Assist if you already have a trained .pt model.",
                    "3. Use Few-Shot Assist after labeling several images in the same scene or domain.",
                    "4. Review suggestions, then accept, reject, or drag-adjust them.",
                ]
            )
        )
        self.few_shot_details = QTextEdit()
        self.few_shot_details.setReadOnly(True)
        self.few_shot_details.setPlainText("Few-shot assist details will appear here after you run it.")
        self.dataset_info = QLabel("Dataset: 0 images")
        self.dataset_info.setWordWrap(True)
        self.assist_status = QLabel("Assist status: detector none | support pool 0 labeled images")
        ...
        self.support_pool_size = QSpinBox()
        self.support_pool_size.setRange(1, 20)
        self.support_pool_size.setValue(5)
        self.support_pool_size.setPrefix("Top supports ")
        self.support_pool_size.valueChanged.connect(self.refresh_few_shot_details)
        self.support_pool_size.valueChanged.connect(self.refresh_summary_labels)
        ...
        self.sort_mode.addItem("Labeled First", "labeled_first")
        self.sort_mode.addItem("Unlabeled First", "unlabeled_first")
        self.sort_mode.addItem("Reviewed First", "reviewed_first")
        ...
        self.auto_few_shot_check = QCheckBox("Auto-run few-shot assist on empty image")
        self.auto_few_shot_check.setChecked(False)
        ...
        self.auto_few_shot_check.toggled.connect(self.schedule_settings_save)
        ...
        self.support_pool_size.valueChanged.connect(self.schedule_settings_save)
    ...
    def setup_layout(self):
        ...
        reference_assist_btn = QPushButton("Run Few-Shot")
        reference_assist_btn.clicked.connect(self.few_shot_assist_from_labeled_pool)
        sam_help_btn = QPushButton("Backend Info")
        sam_help_btn.clicked.connect(self.show_sam_help_dialog)
        ...
        self.action_buttons = {
            ...
            "run_few_shot": reference_assist_btn,
            "fit": fit_btn,
        ...
        reference_group = QGroupBox("Few-Shot Assist")
        reference_layout = QVBoxLayout(reference_group)
        reference_layout.addWidget(self.assist_status)
        reference_layout.addWidget(self.support_pool_size)
        reference_layout.addWidget(reference_assist_btn)
        reference_layout.addWidget(self.auto_few_shot_check)
        reference_layout.addWidget(self.few_shot_details)
        assist_layout.addWidget(reference_group)
        fewshot_group = QGroupBox("Few-Shot Assist")
        fewshot_layout = QVBoxLayout(fewshot_group)
        fewshot_layout.addWidget(self.few_shot_status)
        fewshot_layout.addWidget(sam_help_btn)
        assist_layout.addWidget(fewshot_group)
        notes_group = QGroupBox("How Assist Works")
        notes_layout = QVBoxLayout(notes_group)
        notes_layout.addWidget(self.assist_notes)
        assist_layout.addWidget(notes_group)

        tabs = QTabWidget()
        tabs.addTab(project_tab, "Project")
        tabs.addTab(workbench_tab, "Review + Auto")
        tabs.addTab(assist_tab, "Assist")
        assist_index = tabs.indexOf(assist_tab)
        if assist_index >= 0:
            tabs.setTabEnabled(assist_index, False)
    ...
    def setup_actions(self):
        ...
        file_menu.addAction(unload_model)
        file_menu.addAction(export_config)
        file_menu.addAction(import_config)
        file_menu.addAction(reset_settings)
        file_menu.addAction(configure_shortcuts)
    ...
    def install_shortcuts(self):
        ...
        bindings = [
            ...
            ("run_few_shot", self.few_shot_assist_from_labeled_pool),
            ("nudge_left", lambda: self.canvas.nudge_selected(-1, 0)),
            ("nudge_right", lambda: self.canvas.nudge_selected(1, 0)),
            ("nudge_up", lambda: self.canvas.nudge_selected(0, -1)),
            ("nudge_down", lambda: self.canvas.nudge_selected(0, 1)),
            ("nudge_left_fast", lambda: self.canvas.nudge_selected(-10, 0)),
            ("nudge_right_fast", lambda: self.canvas.nudge_selected(10, 0)),
            ("nudge_up_fast", lambda: self.canvas.nudge_selected(0, -10)),
            ("nudge_down_fast", lambda: self.canvas.nudge_selected(0, 10)),
        ...
        for number in range(1, 10):
            ...
            shortcut.setContext(Qt.ApplicationShortcut)
            shortcut.activated.connect(partial(self.select_class_by_index, number - 1))
            self.shortcuts.append(shortcut)
        self.apply_button_shortcuts()
    ...
    def apply_button_shortcuts(self):
        ...
        for key, button in self.action_buttons.items():
            ...
            full_labels = {
                ...
                "run_few_shot": "Run few-shot assist",
    ...
    def apply_sort_mode(self):
        mode = self.sort_mode.currentData() or "name_asc"
        if mode == "name_desc":
            self.image_files = sorted(self.image_files, reverse=True)
        elif mode == "name_natural":
            self.image_files = sorted(self.image_files, key=self.natural_sort_key)
        elif mode == "labeled_first":
            self.image_files = sorted(self.image_files, key=lambda name: (not self.has_saved_labels(name), name.lower()))
        elif mode == "unlabeled_first":
            self.image_files = sorted(self.image_files, key=lambda name: (self.has_saved_labels(name), name.lower()))
        elif mode == "reviewed_first":
            self.image_files = sorted(self.image_files, key=lambda name: (not self.is_reviewed_image(name), name.lower()))
        else:
    ...
    def save_settings(self):
        ...
        self.settings.setValue("auto_few_shot", self.auto_few_shot_check.isChecked())
        ...
        self.settings.setValue("support_pool_size", self.support_pool_size.value())
    ...
    def restore_settings(self):
        ...
        self.auto_few_shot_check.setChecked(self.settings.value("auto_few_shot", False, type=bool))
        ...
        self.support_pool_size.setValue(self.settings.value("support_pool_size", 5, type=int))
    ...
    def export_config_xml(self):
        ...
        values = {
            ...
            "auto_few_shot": str(self.auto_few_shot_check.isChecked()).lower(),
            ...
            "support_pool_size": str(self.support_pool_size.value()),
            "import_model_classes": str(self.import_model_classes_check.isChecked()).lower(),
    ...
    def import_config_xml(self):
        ...
        self.auto_few_shot_check.setChecked(values.get("auto_few_shot", str(self.auto_few_shot_check.isChecked()).lower()) == "true")
        ...
        try:
            ...
            pass
        try:
            self.support_pool_size.setValue(int(values.get("support_pool_size", self.support_pool_size.value())))
        except (TypeError, ValueError):
            pass
        try:
    ...
    def show_sam_help_dialog(self):
        QMessageBox.information(
            self,
            "Few-Shot Backend Info",
            "\n".join(
                [
                    "Real Few-Shot Detection Backends",
                    "",
                    "Examples from official projects:",
                    "- MMFewShot",
                    "- FSCE",
                    "- Meta-DETR",
                    "",
                    "These are not embedded in this desktop app yet.",
                    "They usually require separate research frameworks such as Detectron2 or MMCV/MMDetection.",
                    "",
                    "This app currently offers:",
                    "- detector-based assist via your loaded .pt model",
                    "- reference-image transfer assist",
                    "- detector fine-tuning on labeled images",
                ]
            ),
        )
    ...
    def refresh_summary_labels(self):
        ...
        detector_status = os.path.basename(self.model_path) if self.detector and self.model_path else "none"
        self.assist_status.setText(
            f"Assist status: detector {detector_status} | support pool {self.labeled_count} labeled images"
        )
        self.few_shot_status.setText(
            f"Few-shot assist uses the best {min(self.support_pool_size.value(), max(self.labeled_count - 1, 0))} labeled matches as supports"
        )
    ...
    def refresh_file_list(self, preserve_row=True):
        ...
        self.file_list.blockSignals(False)
        self.refresh_summary_labels()
        if hasattr(self, "tabs") and self.tabs.currentWidget() is not None and self.tabs.tabText(self.tabs.currentIndex()) == "Assist":
            self.refresh_few_shot_details()
    ...
    def refresh_few_shot_details(self, chosen_supports=None, ranked_supports=None, suggestions=None):
        if not isinstance(chosen_supports, (list, tuple, set)):
            chosen_supports = None
        if not isinstance(ranked_supports, (list, tuple)):
            ranked_supports = None
        if not isinstance(suggestions, (list, tuple)):
            suggestions = None
        current_name = self.current_image_name() or "none"
        support_images = self.labeled_support_images()
        support_count = len(support_images)
        pool_counts, total_pool_labels = self.label_counts_for_images(support_images)
        lines = [
            "Few-shot assist details",
            f"Current image: {current_name}",
            "Mode: support-image transfer from labeled project images",
            f"Labeled support pool: {support_count} images",
            f"Configured top supports: {self.support_pool_size.value()}",
            f"Labels available in pool: {total_pool_labels}",
            f"Pool class totals: {self.format_label_counts(pool_counts)}",
        ]
        if chosen_supports is None:
            preview = support_images[: min(8, len(support_images))]
            lines.append("")
            lines.append("Available support images:")
            if preview:
                lines.extend(f"- {image_name}" for image_name in preview)
                if len(support_images) > len(preview):
                    lines.append(f"- ... and {len(support_images) - len(preview)} more")
            else:
                lines.append("- none")
            self.few_shot_details.setPlainText("\n".join(lines))
            return

        chosen_counts, chosen_total_labels = self.label_counts_for_images(chosen_supports)
        lines.extend(
            [
                "",
                f"Supports used this run: {len(chosen_supports)}",
                f"Labels in chosen supports: {chosen_total_labels}",
                f"Chosen support class totals: {self.format_label_counts(chosen_counts)}",
            ]
        )
        if ranked_supports:
            match_values = [match_count for match_count, _ in ranked_supports]
            lines.extend(
                [
                    f"Match count range: {min(match_values)} to {max(match_values)}",
                    f"Average match count: {sum(match_values) / max(len(match_values), 1):.1f}",
                ]
            )
        suggestion_counts = {}
        for suggestion in suggestions or []:
            suggestion_counts[suggestion.class_id] = suggestion_counts.get(suggestion.class_id, 0) + 1
        lines.extend(
            [
                f"Suggestions produced this run: {len(suggestions or [])}",
                f"Suggested class totals: {self.format_label_counts(suggestion_counts)}",
                "",
                "Selected support images:",
            ]
        )
        if ranked_supports:
            for match_count, image_name in ranked_supports:
                label_count = self.get_label_summary(image_name)["total"]
                lines.append(f"- {image_name} | matches {match_count} | labels {label_count}")
        else:
            lines.append("- none")
        self.few_shot_details.setPlainText("\n".join(lines))
    ...
    def load_current_image(self, row):
        ...
        if self.auto_few_shot_check.isChecked() and not self.canvas.annotations:
            self.few_shot_assist_from_labeled_pool(silent=True)
    ...
    def labeled_support_images(self):
        current_name = self.current_image_name()
        return [image_name for image_name in self.image_files if image_name != current_name and self.has_saved_labels(image_name)]

    def few_shot_assist_from_labeled_pool(self, silent=False):
        support_images = self.labeled_support_images()
        if not support_images:
            self.few_shot_status.setText("Few-shot assist: no labeled support images available")
            self.refresh_few_shot_details([], [], [])
            if not silent:
                self.status.showMessage("No labeled support images available for few-shot assist.", 4000)
            return

        current_features = self.get_image_features(self.current_image_path())
        if not current_features:
            self.few_shot_status.setText("Few-shot assist: current image features could not be computed")
            self.refresh_few_shot_details([], [], [])
            if not silent:
                self.status.showMessage("Few-shot assist could not read or analyze the current image.", 4000)
            return

        ranked = []
        for index, image_name in enumerate(support_images, start=1):
            reference_path = os.path.join(self.folder, image_name)
            reference_features = self.get_image_features(reference_path)
            if not reference_features:
                continue
            match_count = self.count_reference_matches(image_name, current_features, reference_features)
            if match_count > 0:
                ranked.append((match_count, image_name, reference_features))
            if index % 8 == 0:
                QApplication.processEvents()
                if not silent:
                    self.status.showMessage(f"Few-shot assist: scored {index}/{len(support_images)} support images...", 1500)
        ranked.sort(key=lambda item: (item[0], item[1].lower()), reverse=True)
        chosen_entries = ranked[: self.support_pool_size.value()]
        chosen = [image_name for _, image_name, _reference_features in chosen_entries]
        if not chosen:
            self.few_shot_status.setText("Few-shot assist: no visually similar labeled images found")
            self.refresh_few_shot_details([], [], [])
            if not silent:
                self.status.showMessage("No visually similar labeled images found for few-shot assist.", 4000)
            return

        combined = []
        base_annotations = [] if self.replace_labels.isChecked() else list(self.canvas.annotations)
        ...
        for _match_count, image_name, reference_features in chosen_entries:
            try:
                transferred = self.suggest_from_reference_image(image_name, current_features, reference_features)
            except Exception:
                continue
            filtered = self.filter_new_detections(
                [(item.class_id, item.x1, item.y1, item.x2, item.y2, item.score) for item in transferred],
                base_annotations + existing_suggestions + combined,
                roi_points,
            )
            combined.extend(filtered)

        self.canvas.set_suggestions(existing_suggestions + combined)
        self.refresh_few_shot_details(chosen, [(match_count, image_name) for match_count, image_name, _reference_features in chosen_entries], combined)
        self.few_shot_status.setText(
            f"Few-shot assist: {len(combined)} suggestions from {len(chosen)} support images"
        )
        if not silent:
            self.status.showMessage(
                f"Few-shot assist added {len(combined)} suggestions using {len(chosen)} support images.",
                5000,
            )
    ...
    def eventFilter(self, obj, event):
        if obj in (self.class_list, self.class_picker) and event.type() == QEvent.KeyPress:
            if event.key() == Qt.Key_Escape:
