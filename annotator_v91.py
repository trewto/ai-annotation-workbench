...
class ShortcutDialog(QDialog):
    def __init__(self, shortcuts, parent=None):
        ...
        info = QLabel("Mouse shortcut: Alt+Left Click cycles overlapping real boxes under cursor.")
...
class ClassReorderDialog(QDialog):
    def __init__(self, classes, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Reorder Classes")
        self.resize(420, 520)
        self.original_classes = list(classes)

        layout = QVBoxLayout(self)
        info = QLabel("Drag to reorder classes. Changes will remap labels to match the new order.")
        info.setWordWrap(True)
        layout.addWidget(info)

        self.list_widget = QListWidget()
        self.list_widget.setDragDropMode(QAbstractItemView.InternalMove)
        for index, name in enumerate(self.original_classes):
        ...
        sort_btn.clicked.connect(self.sort_az)
    ...
    def sort_az(self):
        items = [(self.list_widget.item(i).text(), self.list_widget.item(i).data(Qt.UserRole)) for i in range(self.list_widget.count())]
        items.sort(key=lambda pair: pair[0].lower())
        self.list_widget.clear()
        for text, old_index in items:
            item = QListWidgetItem(text)
            item.setData(Qt.UserRole, old_index)
            self.list_widget.addItem(item)
    ...
    def mapping(self):
        mapping = {}
        new_classes = []
        for new_index in range(self.list_widget.count()):
            item = self.list_widget.item(new_index)
            old_index = item.data(Qt.UserRole)
            mapping[int(old_index)] = new_index
            new_classes.append(item.text())
        return mapping, new_classes


class ClassReorderPreviewDialog(QDialog):
    def __init__(self, text, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Class Reorder Preview")
        self.resize(520, 560)
        layout = QVBoxLayout(self)
        info = QLabel("Dry-run preview. Review the changes below before applying.")
        info.setWordWrap(True)
        layout.addWidget(info)
        editor = QTextEdit()
...
class Canvas(QWidget):
    ...
    def __init__(self):
        ...
        self.force_new_box_shortcut_active = False
        self.history = []
        self.redo_history = []
        self.max_history = 200
        self.suggestions = []
        self.suggestions_from_model = False
        self.model_conflict_hints = []
        self.arrow_keys_navigate_images = False
    ...
    def snapshot_state(self):
        return {
            ...
            "suggestions_from_model": self.suggestions_from_model,
            "model_conflict_hints": self.clone_annotations(self.model_conflict_hints),
    ...
    def restore_state(self, state):
        ...
        self.suggestions_from_model = bool(state.get("suggestions_from_model", False))
        self.model_conflict_hints = self.clone_annotations(state.get("model_conflict_hints", []))
    ...
    def set_suggestions(self, suggestions, from_model=False, conflict_hints=None):
        self.suggestions = suggestions
        self.suggestions_from_model = bool(from_model)
        self.model_conflict_hints = self.clone_annotations(conflict_hints or [])
    ...
    def conflicts_with_model_suggestion(self, annotation):
        if not self.model_conflict_hints:
            return False
        for hint in self.model_conflict_hints:
            if hint.class_id != annotation.class_id and self.annotation_iou(annotation, hint) >= 0.45:
                return True
        return False
    ...
    def paintEvent(self, _event):
        ...
        if self.show_boxes:
            for index, annotation in enumerate(self.annotations):
                conflict = self.conflicts_with_model_suggestion(annotation)
                self.draw_annotation(painter, annotation, index in self.selected_indices, conflict=conflict)
    ...
    def draw_annotation(self, painter, annotation, is_selected, preview=False, conflict=False):
        box = annotation.normalized()
        ...
        if conflict and not preview:
            color = QColor(255, 0, 180)
        painter.setBrush(Qt.NoBrush)
        painter.setPen(QPen(color, 4 if conflict else 3 if is_selected else 2))
        painter.drawRect(rect)
        if conflict and not preview:
            painter.fillRect(rect, QColor(255, 0, 180, 45))
            painter.setPen(QPen(QColor(255, 255, 255), 2, Qt.DashLine))
            painter.drawRect(rect.adjusted(3, 3, -3, -3))
            painter.setPen(QPen(QColor(255, 230, 0), 2, Qt.DotLine))
        ...
        if conflict and not preview:
            painter.fillRect(text_rect, QColor(35, 0, 25, 230))
    ...
    def mousePressEvent(self, event):
        ...
        if event.modifiers() & Qt.AltModifier:
            overlap_hits = self.annotations_at(image_x, image_y)
            if overlap_hits:
                if self.selected_index in overlap_hits:
                    current_pos = overlap_hits.index(self.selected_index)
                    next_index = overlap_hits[(current_pos + 1) % len(overlap_hits)]
                else:
                    next_index = overlap_hits[0]
                self.set_selected_index(next_index)
                self.update()
                return

        suggestion_index = self.suggestion_at(image_x, image_y)
        if suggestion_index >= 0 and not self.force_new_box_mode:
            self.selected_suggestion_index = suggestion_index
            self.set_selected_index(-1)
    ...
    def keyPressEvent(self, event):
        ...
        if event.modifiers() & Qt.KeypadModifier and event.key() in keypad_classes:
            self.set_selected_class_id(keypad_classes[event.key()])
            return
        ...
        if event.key() in (Qt.Key_Left, Qt.Key_Right, Qt.Key_Up, Qt.Key_Down):
            if self.space_pressed:
                ...
                self.pan_view(dx, dy)
                return
            plain_arrow = not (event.modifiers() & (Qt.ControlModifier | Qt.AltModifier | Qt.ShiftModifier))
            if self.arrow_keys_navigate_images and plain_arrow:
    ...
    def set_selected_class_id(self, class_id):
        if not (0 <= class_id < len(self.classes)):
            return False
        indices = sorted(self.selected_indices) if self.selected_indices else ([self.selected_index] if self.selected_index >= 0 else [])
        indices = [index for index in indices if 0 <= index < len(self.annotations)]
        if not indices:
            return False
        if all(self.annotations[index].class_id == class_id for index in indices):
...
class Annotator(QMainWindow):
    def __init__(self):
        super().__init__()
        self.base_title = "YOLO Advanced Annotator Pro v91 (Arnob)"
        ...
        self.loaded_class_file_found = False
        ...
        self.importing_model_classes = False
        ...
        self.shortcut_info = QLabel(
            "Draw: drag | Pan: Space+drag | New box over old: hold B | Cycle overlap: Alt+Left Click | Undo/Redo: Z/Y | Reset: C"
        )
        ...
        self.large_project_check = QCheckBox("Large project mode")
        self.large_project_check.setChecked(False)
        self.large_project_check.toggled.connect(self.on_large_project_mode_changed)
        ...
        self.lock_classes_check = QCheckBox("Lock classes")
        self.lock_classes_check.setChecked(False)
        self.lock_classes_check.toggled.connect(self.on_lock_classes_toggled)
        ...
        self.keyboard_review_check.setVisible(False)
        self.arrow_nav_check = QCheckBox("Left/Right change image")
        self.arrow_nav_check.setChecked(False)
        self.arrow_nav_check.setToolTip(
            "When enabled, plain Left/Right moves to the previous/next image. When off, arrows nudge the selected box."
        )
        self.arrow_nav_check.toggled.connect(self.set_arrow_key_navigation)
        ...
        self.import_model_classes_check.setToolTip(
            "When enabled, any selected model classes missing from the project will be added before suggesting.\n"
            "If classes are locked, imports are skipped."
        )
        ...
        self.large_project_check.toggled.connect(self.schedule_settings_save)
        ...
        self.arrow_nav_check.toggled.connect(self.schedule_settings_save)
        self.auto_next_suggest_check.toggled.connect(self.schedule_settings_save)
        self.lock_classes_check.toggled.connect(self.schedule_settings_save)
        ...
        if self.model_path and YOLO is not None and os.path.exists(self.model_path):
            try:
                ...
                self.import_model_classes_after_load()
            except Exception as exc:
                self.detector = None
    ...
    def setup_layout(self):
        ...
        left_layout.addWidget(self.large_project_check)
        nav_row = QHBoxLayout()
        ...
        left_layout.addWidget(self.arrow_nav_check)
        ...
        project_group_layout.addWidget(self.lock_classes_check)
    ...
    def setup_actions(self):
        ...
        dataset_integrity_csv = QAction("Dataset Integrity (CSV)", self)
        dataset_integrity_csv.triggered.connect(self.run_dataset_integrity_check_csv)
        fix_out_of_range = QAction("Fix Out-of-Range Labels", self)
        fix_out_of_range.triggered.connect(self.fix_out_of_range_labels)
        reorder_classes = QAction("Reorder Classes", self)
        reorder_classes.triggered.connect(self.reorder_classes)
        class_map_report = QAction("Class Map Report", self)
        class_map_report.triggered.connect(self.show_class_map_report)
        backup_labels = QAction("Backup Labels Now", self)
        backup_labels.triggered.connect(self.backup_label_files)
        ...
        view_log = QAction("View Error Log", self)
        view_log.triggered.connect(self.show_error_log)
        ...
        tools_menu.addAction(dataset_integrity_csv)
        tools_menu.addAction(fix_out_of_range)
        tools_menu.addAction(backup_labels)
        tools_menu.addAction(reorder_classes)
        tools_menu.addAction(class_map_report)
    ...
    def install_shortcuts(self):
        ...
        bindings = [
            ...
            ("force_new_box", self.toggle_force_new_box_shortcut),
        ...
        for class_id in range(10):
            sequence = QKeySequence(f"Num+{class_id}").toString(QKeySequence.NativeText)
            if sequence in seen_sequences:
                continue
            seen_sequences.add(sequence)
            shortcut = QShortcut(QKeySequence(f"Num+{class_id}"), self)
            shortcut.setContext(Qt.ApplicationShortcut)
            shortcut.activated.connect(self.guard_shortcut(partial(self.set_selected_labels_to_class_id, class_id)))
            self.shortcuts.append(shortcut)
    ...
    def toggle_force_new_box_shortcut(self):
        if not self.canvas.has_image():
            return
        self.canvas.force_new_box_mode = True
        self.canvas.force_new_box_shortcut_active = True
        self.canvas.setCursor(Qt.CrossCursor)

    def release_force_new_box_shortcut(self):
        if not self.canvas.force_new_box_shortcut_active:
            return
        self.canvas.force_new_box_mode = False
        self.canvas.force_new_box_shortcut_active = False
    ...
    def save_settings(self):
        ...
        self.settings.setValue("arrow_nav_images", self.arrow_nav_check.isChecked())
        ...
        self.settings.setValue("lock_classes", self.lock_classes_check.isChecked())
        self.settings.setValue("large_project_mode", self.large_project_check.isChecked())
    ...
    def restore_settings(self):
        ...
        self.arrow_nav_check.setChecked(self.settings.value("arrow_nav_images", False, type=bool))
        ...
        self.lock_classes_check.setChecked(self.settings.value("lock_classes", False, type=bool))
        self.large_project_check.setChecked(self.settings.value("large_project_mode", False, type=bool))
    ...
    def export_config_xml(self):
        ...
        values = {
            ...
            "lock_classes": str(self.lock_classes_check.isChecked()).lower(),
            "large_project_mode": str(self.large_project_check.isChecked()).lower(),
    ...
    def import_config_xml(self):
        ...
        self.lock_classes_check.setChecked(values.get("lock_classes", str(self.lock_classes_check.isChecked()).lower()) == "true")
        self.large_project_check.setChecked(values.get("large_project_mode", str(self.large_project_check.isChecked()).lower()) == "true")
        try:
    ...
    def run_dataset_integrity_check_csv(self):
        if not self.folder or not self.image_files:
            QMessageBox.information(self, "Dataset Integrity", "Open a project folder first.")
            return
        path, _ = QFileDialog.getSaveFileName(
            ...
            os.path.join(self.folder, "dataset_integrity.csv"),
            "CSV Files (*.csv)",
        )
        if not path:
            return
        ...
        valid_label_files = set()
        class_limit = len(self.canvas.classes)
        progress = QProgressDialog("Integrity check...", "", 0, max(len(self.image_files), 1), self)
        ...
        try:
            for index, image_name in enumerate(self.image_files, start=1):
                ...
                percent = index * 100 // max(len(self.image_files), 1)
                progress.setLabelText(f"Checking {image_name} ({index}/{len(self.image_files)}) - {percent}%")
                QApplication.processEvents()
                ...
                with open(label_path, "r", encoding="utf-8") as handle:
                    for line_number, line in enumerate(handle, start=1):
                        ...
                        if class_id < 0 or class_id >= class_limit:
                            out_of_range_classes.append(f"{label_name}:{line_number}")
                        if bw <= 0 or bh <= 0:
                            malformed_labels.append(f"{label_name}:{line_number}")
                            continue
                        if not (0 <= xc <= 1 and 0 <= yc <= 1 and 0 < bw <= 1 and 0 < bh <= 1):
        ...
        with open(path, "w", newline="", encoding="utf-8") as handle:
            ...
            writer.writerow(["Issue", "Item"])
            for name in unreadable_images:
                writer.writerow(["Unreadable image", name])
            for item in malformed_labels:
                writer.writerow(["Malformed label", item])
            for item in out_of_range_classes:
                writer.writerow(["Out-of-range class", item])
            for name in orphan_labels:
                writer.writerow(["Orphan label", name])
        QMessageBox.information(
            self,
            "Dataset Integrity",
            f"Integrity report exported to {os.path.basename(path)}",
        )
        ...
        text.setPlainText(
            "\n".join(
                [
                    ...
                    "- Alt+Left Click cycles through overlapping real boxes under cursor.",
    ...
    def show_error_log(self):
        ...
        if not os.path.exists(crash_log_path):
            QMessageBox.information(self, "Error Log", "No crash log found yet.")
            return
        dialog = QDialog(self)
        ...
        text.setReadOnly(True)
        try:
            with open(crash_log_path, "r", encoding="utf-8") as handle:
                text.setPlainText(handle.read())
        except OSError as exc:
            text.setPlainText(f"Could not read log: {exc}")
        layout.addWidget(text)
        buttons = QDialogButtonBox(QDialogButtonBox.Close)
        buttons.rejected.connect(dialog.reject)
        buttons.accepted.connect(dialog.accept)
    ...
    def sync_images_with_disk(self, reload_current=True, notify=False, force=False):
        if not self.folder or not os.path.isdir(self.folder):
            return False
        if getattr(self, "large_project_check", None) is not None and self.large_project_check.isChecked() and not force:
            return False
        now = time.monotonic()
    ...
    def load_classes(self):
        ...
        for name in CLASS_FILE_NAMES:
            ...
            if os.path.exists(candidate):
                ...
                self.loaded_class_file_found = True
    ...
    def set_arrow_key_navigation(self, checked):
        self.canvas.arrow_keys_navigate_images = checked
    ...
    def delete_labels_for_class_id(self, class_id):
        if class_id < 0:
            return
        summary = self.remap_labels_in_dataset_internal(class_id, remap_to=None, dry_run=False, shift_ids=False)
        if self.canvas.has_image():
            updated = [annotation for annotation in self.canvas.annotations if annotation.class_id != class_id]
            if len(updated) != len(self.canvas.annotations):
                self.canvas.annotations = updated
                self.canvas.annotations_changed.emit()
                self.canvas.update()
        self.refresh_summary_labels()
        return summary
    ...
    def remap_labels_in_dataset(self, removed_id, remap_to=None):
        return self.remap_labels_in_dataset_internal(removed_id, remap_to, dry_run=False, shift_ids=True)

    def remap_labels_in_dataset_internal(self, removed_id, remap_to=None, dry_run=False, shift_ids=True):
        summary = {
            "files_scanned": 0,
            "files_updated": 0,
            "labels_removed": 0,
        ...
        try:
            for index, image_name in enumerate(self.image_files, start=1):
                if progress:
                    progress.setValue(index - 1)
                    percent = index * 100 // max(len(self.image_files), 1)
                    progress.setLabelText(f"Updating {image_name} ({index}/{len(self.image_files)}) - {percent}%")
                    QApplication.processEvents()
                    if progress.wasCanceled():
                ...
                with open(label_path, "r", encoding="utf-8") as handle:
                    for line in handle:
                        ...
                        elif shift_ids and class_id > removed_id:
                            class_id -= 1
                            updated = True
        ...
        if not dry_run and self.canvas.has_image():
            ...
            for annotation in self.canvas.annotations:
                ...
                elif shift_ids and annotation.class_id > removed_id:
    ...
    def remap_labels_with_mapping_internal(self, mapping, dry_run=False):
        summary = {
            "files_scanned": 0,
            "files_updated": 0,
            "labels_remapped": 0,
        ...
        progress = None
        if not dry_run:
            progress = QProgressDialog("Remapping labels...", "", 0, max(len(self.image_files), 1), self)
            progress.setWindowModality(Qt.WindowModal)
            progress.setCancelButtonText("Cancel")
            progress.setMinimumDuration(250)
            progress.show()
        ...
        try:
            for index, image_name in enumerate(self.image_files, start=1):
                if progress:
                    progress.setValue(index - 1)
                    percent = index * 100 // max(len(self.image_files), 1)
                    progress.setLabelText(f"Remapping {image_name} ({index}/{len(self.image_files)}) - {percent}%")
                    QApplication.processEvents()
                    if progress.wasCanceled():
                        summary["errors"].append("Operation canceled by user.")
                        break
    ...
    def show_class_map_report(self):
        lines = []
        lines.append("Class Map Report")
        lines.append("")
        project_classes = list(self.canvas.classes)
        lines.append(f"Project classes ({len(project_classes)}):")
        if project_classes:
            ...
            lines.append("- (none)")
        lines.append("")
        model_classes = list(self.model_class_names) if self.model_class_names else []
        lines.append(f"Model classes ({len(model_classes)}):")
        if model_classes:
        ...
        lines.append("")
        if model_classes:
            lines.append("Model → Project mapping:")
            mapped = 0
            for index, name in enumerate(model_classes):
                mapped_id = self.model_class_map.get(index)
                if mapped_id is None:
                    lines.append(f"- {index}:{name} -> (unmapped)")
                else:
                    mapped += 1
                    target = self.canvas.class_name(mapped_id)
                    lines.append(f"- {index}:{name} -> {mapped_id}:{target}")
            lines.append(f"Mapped {mapped}/{len(model_classes)} model classes.")
        counts = self.dataset_class_counts()
        if counts:
            lines.append("")
            lines.append("Label counts by class:")
            for class_id, count in sorted(counts.items()):
                lines.append(f"- {class_id}:{self.canvas.class_name(class_id)} = {count}")
        dialog = QDialog(self)
        dialog.setWindowTitle("Class Map Report")
    ...
    def on_lock_classes_toggled(self, checked):
        self.class_sort_mode.setEnabled(not checked)
        self.class_list.setEnabled(True)
        self.class_picker.setEnabled(True)
        if checked:
            self.import_model_classes_check.setToolTip(
                "Classes are locked. Model classes cannot be imported.\n"
                "Unlock classes to allow imports."
            )
        else:
            self.import_model_classes_check.setToolTip(
                "When enabled, any selected model classes missing from the project will be added before suggesting.\n"
    ...
    def on_large_project_mode_changed(self, checked):
        if checked:
            self.status.showMessage("Large project mode enabled: reduced background scans.", 4000)
        else:
            self.status.showMessage("Large project mode disabled.", 3000)

    def reorder_classes(self):
        if self.lock_classes_check.isChecked():
            QMessageBox.information(self, "Classes locked", "Unlock classes to add, rename, delete, or reorder.")
            return
        classes = list(self.project_master_classes or self.canvas.classes)
        if len(classes) < 2:
            QMessageBox.information(self, "Reorder Classes", "Need at least two classes to reorder.")
            return
        dialog = ClassReorderDialog(classes, self)
        if dialog.exec_() != QDialog.Accepted:
            return
        mapping, new_classes = dialog.mapping()
        if new_classes == classes:
            return
        counts = self.dataset_class_counts()
        summary = self.remap_labels_with_mapping_internal(mapping, dry_run=True)
        lines = [
            "Dry-run summary:",
        ...
        for old_id, old_name in enumerate(classes):
            ...
            new_name = new_classes[new_id] if new_id < len(new_classes) else "unknown"
            count = counts.get(old_id, 0)
            lines.append(f"- {old_id}:{old_name} ({count}) -> {new_id}:{new_name}")
        if summary["errors"]:
            lines.append("")
            lines.append("Warnings:")
            lines.extend(summary["errors"][:6])
        preview = ClassReorderPreviewDialog("\n".join(lines), self)
        if preview.exec_() != QDialog.Accepted:
            return
        confirm = QMessageBox.question(
            self,
            "Reorder Classes",
            "This will rewrite label files to match the new class order.\n"
            "A backup will be created, but the change is irreversible without restoring.\n\n"
            "Proceed?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if confirm != QMessageBox.Yes:
            return
        self.backup_label_files()
        self.project_master_classes = list(new_classes)
        self.populate_classes(list(new_classes))
        self.save_classes(self.project_master_classes)
        self.backup_project_classes(force=True)
        self.remap_labels_with_mapping_internal(mapping, dry_run=False)
        self.status.showMessage("Classes reordered and labels remapped.", 4000)

    def confirm_dataset_class_change(self, action_label, removed_id, remap_to=None, removed_name="", target_name=""):
        summary = self.remap_labels_in_dataset_internal(removed_id, remap_to, dry_run=True, shift_ids=True)
        extra = ""
        if target_name:
    ...
    def backup_label_files(self):
        return self.backup_labels_before_change()

    def restore_labels_from_backup(self):
        if not self.folder:
            QMessageBox.warning(self, "Restore failed", "Open a project folder first.")
            return
        backup_root = os.path.join(self.folder, "_label_backups")
        if not os.path.isdir(backup_root):
            QMessageBox.information(self, "Restore", "No backups found for this project.")
            return
        candidates = [entry.name for entry in os.scandir(backup_root) if entry.is_dir()]
        if not candidates:
            QMessageBox.information(self, "Restore", "No backups found for this project.")
            return
        ...
        answer = QMessageBox.question(
            self,
            "Restore Labels",
            "This will replace all label .txt files in the project with the backup copy.\n"
            "It will NOT change classes.txt.\n\n"
            "Continue?",
            QMessageBox.Yes | QMessageBox.No,
            QMessageBox.No,
        )
        if answer != QMessageBox.Yes:
            return
        backup_dir = os.path.join(backup_root, choice)
        if not os.path.isdir(backup_dir):
            QMessageBox.warning(self, "Restore failed", "Selected backup folder is missing.")
            return
        removed = 0
        restored = 0
        for entry in os.scandir(self.folder):
            if entry.is_file() and entry.name.lower().endswith(".txt") and entry.name not in {
                "classes.txt",
                "obj.names",
                "classes.project_backup.txt",
            }:
                try:
        ...
        for entry in os.scandir(backup_dir):
            if entry.is_file() and entry.name.lower().endswith(".txt"):
                try:
                    shutil.copy2(entry.path, os.path.join(self.folder, entry.name))
                    restored += 1
                except OSError:
                    continue
        self.refresh_dataset_cache()
        self.apply_sort_mode()
        self.refresh_file_list(preserve_row=False)
        self.status.showMessage(f"Restored {restored} label files from backup ({removed} removed).", 5000)
    ...
    def ensure_model_classes_imported(self, allowed_model_names=None):
        ...
        if self.lock_classes_check.isChecked():
            self.status.showMessage(
                "Classes are locked. Unmapped model classes will be skipped. Unlock to import.",
                4000,
            )
            return
    ...
    def import_model_classes_after_load(self):
        if self.importing_model_classes:
            return
        if not (self.detector and self.model_class_names):
            return
        if not self.import_model_classes_check.isChecked():
            return
        if self.lock_classes_check.isChecked():
            self.status.showMessage("Classes are locked. Model classes were not imported.", 4000)
            return
        project_has_classes = bool(self.project_master_classes or self.canvas.classes)
        missing_mapped_classes = any(mapped_index is None for mapped_index in self.model_class_map.values())
        if project_has_classes and self.loaded_class_file_found and not missing_mapped_classes:
            return
        before_count = len(self.project_master_classes or self.canvas.classes)
        try:
            self.importing_model_classes = True
            self.ensure_model_classes_imported()
        finally:
            self.importing_model_classes = False
        after_count = len(self.project_master_classes or self.canvas.classes)
        if after_count > before_count:
            self.status.showMessage(
                f"Imported {after_count - before_count} model classes into classes.txt.",
                5000,
            )
    ...
    def build_model_conflict_hints(self, detections, annotations, roi_points=None, allowed_class_ids=None):
        hints = []
        iou_limit = self.iou_spin.value()
        for annotation in annotations:
            best_candidate = None
            ...
            for detection in detections:
                ...
                if not self.inside_roi(candidate, roi_points):
                    continue
                iou = self.annotation_iou(candidate, annotation)
                if iou < iou_limit:
                    continue
                rank = (score, iou)
    ...
    def auto_annotate_current(self):
        ...
        # When locked, suggestions will skip unmapped classes (tooltip communicates this).
        if self.model_class_names and not any(value is not None for value in self.model_class_map.values()):
            if self.import_model_classes_check.isChecked() and selected_model_names:
                self.ensure_model_classes_imported(selected_model_names)
            elif self.import_model_classes_check.isChecked() and not selected_model_names:
                self.status.showMessage(
                    "No classes selected in Suggest Classes. Select one or more classes to import.",
                    4000,
                )
        ...
        if self.import_model_classes_check.isChecked():
            if selected_model_names:
                ...
                self.status.showMessage(
                    "Select classes in Suggest Classes to import only those. No classes were imported.",
                    4000,
                )
        if self.model_class_names and not any(value is not None for value in self.model_class_map.values()):
            ...
            QMessageBox.information(
                self,
                "No Mapped Classes",
                "No model classes are mapped to this project yet.\n"
                "Select model classes and enable Auto-import to bring them in.",
            )
            return
        try:
        ...
        conflict_hints = []
        if not self.replace_labels.isChecked():
            conflict_hints = self.build_model_conflict_hints(
                detections,
                self.canvas.annotations,
                roi_points,
                allowed_class_ids,
        ...
        self.canvas.set_suggestions(existing_suggestions + suggestions, from_model=True, conflict_hints=conflict_hints)
    ...
    def set_selected_labels_to_class_id(self, class_id):
        if not (0 <= class_id < len(self.canvas.classes)):
            return
        if not self.canvas.set_selected_class_id(class_id):
            self.status.showMessage("Select one or more labels first.", 3000)
            return
        if self.autosave_on_image_change():
            self.save_current_labels()
        selected_count = len(self.canvas.selected_indices) or (1 if self.canvas.selected_index >= 0 else 0)
        self.status.showMessage(
            f"Changed {selected_count} selected label(s) to {self.canvas.class_name(class_id)}.",
            3000,
        )
    ...
    def keyPressEvent(self, event):
        if event.key() == Qt.Key_B and hasattr(self, "canvas"):
            self.toggle_force_new_box_shortcut()
    ...
    def keyReleaseEvent(self, event):
        if event.key() == Qt.Key_B and hasattr(self, "canvas"):
            self.release_force_new_box_shortcut()
            return
        super().keyReleaseEvent(event)
